...
def monitor_cumulative_throughput(iface, interval_sec = 0.01, fname='%s/cthroughput.txt' % default_dir):
    first = True
    ref = 0
    start_time = time()
    while 1:
        ...
        for line in lines:
            if iface in line:
                break
        f.close()
        bytes_sent = float(line.split()[9])
        if(first):
            ref = bytes_sent
            first = False
        bytes_sent = bytes_sent - ref
        t = time() - start_time
        throughput = bytes_sent / t
        open(fname, 'a').write('%f' % t + ',' + '%d' % throughput + '\n')
        sleep(interval_sec)
    return
