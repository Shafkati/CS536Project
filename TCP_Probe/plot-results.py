...
for f in glob.glob("%s/*regular/result.txt" % args.dir):
    print "Parsing %s" % f
    parse_data(f)
    nfiles += 1
...
plt.plot(keys, avg_mn, lw=2, label="Mininet-regular", color="red", marker='s', markersize=10)

data = defaultdict(list)
for f in glob.glob("%s/*paced/result.txt" % args.dir):
    print "Parsing %s" % f
    parse_data(f)
    nfiles += 1
...
for k in keys:
    avg_mn.append(avg(data[k]))
plt.plot(keys, avg_mn, lw=2, label="Mininet-paced", color="purple", marker='s', markersize=10)
